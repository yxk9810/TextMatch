...
def compute_mse_loss_test(pred0, pred1, pred2):
    ...
    prob = prob_1*0.5+0.5
    label = torch.cat((torch.ones(sim_pos.shape[0]), torch.zeros(sim_pos.shape[0])), 0)
...
def validate(model, dataloader, device, ema=None, output_file=None):
    ...
    with t.no_grad():
        for batch_index, data in enumerate(tqdm_batch_iterator):
            ...
            prob_pos, loss_pos = model(input_ids0, attention_mask0, token_type_ids0, input_ids1, attention_mask1, token_type_ids1, labels)
            all_labels.extend(labels.cpu().tolist())
            all_prob.extend(prob_pos[:,1].cpu().tolist())
            loss = loss_pos
    ...
    epoch_auc = roc_auc_score(all_labels, all_prob)
    return epoch_time, epoch_loss, epoch_auc
...
def train(model, dataloader, optimizer, max_gradient_norm, device, scheduler=None, fgm=None, pgd=None, ema=None):
    model.train()
    epoch_start = time.time()
    batch_time_avg = 0.0
    running_loss = 0.0
    ...
    for batch_index, data in enumerate(tqdm_batch_iterator):
        ...
        # token_type_ids2 = data['token_type_ids'][:,2].to(device)
        optimizer.zero_grad()
        # labels_pos = torch.ones(input_ids0.shape[0], dtype=torch.long).to(device)
        # labels_neg = torch.zeros(input_ids0.shape[0], dtype=torch.long).to(device)
        prob_pos, loss_pos = model(input_ids0, attention_mask0, token_type_ids0, input_ids1, attention_mask1, token_type_ids1, labels)
