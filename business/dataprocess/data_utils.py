...
class MyDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = self.tokenizer([source[0],source[1]],max_length=self.maxlen,truncation=True,padding='max_length',return_tensors='pt')
        sample['label'] = source[2]        
        return sample
...
def load_data(file_path):
    ...
    with open(file_path) as f:
        for idx,line in enumerate(f):
            ...
            sent1, sent2, label = info['sentence1'], info['sentence2'], 1 if 'entail' in info['gold_label'] else 0
            res_list.append([sent1,sent2,label])          
    return res_list
